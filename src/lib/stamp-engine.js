/* =============================================================================
   THE STAMP ENGINE — docs/launch/code/05-stamp-engine.js, BYTE FOR BYTE.
   -----------------------------------------------------------------------------
   The owner (2026-09-21): "Replace the whole stamp engine with
   docs/launch/code/05-stamp-engine.js (everything from MOTTO down to
   colourGrid). Delete the old one, don't patch it." So everything between the
   two marker lines below is that file, unedited — `npm run check:stamp` fails
   the build if a single byte differs. The old port, which carried its own
   adaptations inside the drawing, is gone.

   What sits OUTSIDE the markers is only what the file assumes of the page it
   was cut from, supplied rather than written into it:

     · FONTS — the reference defines it elsewhere in its page. The faces are
       the app's own tokens' faces (CLAUDE.md: Instrument Sans).
     · ONE <svg><defs> AT THE TOP OF <body>. The engine finds its defs with
       document.querySelector('svg defs'), appends each ink filter there, and
       measures every path it samples inside that same <svg>. The first match
       in document order must be one that React never removes, or a stamp's
       filter disappears with some other component's icon and the stamp stops
       drawing. So it is put first in <body>, before the app's root.
     · The export list at the foot.

   eslint is off for the file: it is a vendored slice, kept in the
   reference's own order and style, and it carries helpers the app does not
   call. Checking it would mean editing it.
   ========================================================================= */
/* eslint-disable */
const FONTS = { normal: 'Instrument Sans,ui-sans-serif,system-ui,sans-serif', cursive: 'Pacifico,Brush Script MT,cursive' };
if (typeof document !== 'undefined' && document.body && !document.getElementById('pw-stamp-defs')) {
  const host = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  host.setAttribute('id', 'pw-stamp-defs');
...
/* ---------------------------------------------- BEGIN docs/launch/code/05-stamp-engine.js */
...
/* ------------------------------------------------ END docs/launch/code/05-stamp-engine.js */
export {
  FONTS, MOTTO, DEFAULT_STAMP, PALETTE, col, SHAPES, MARKS, SYMS, inkVal, inkFilter,
  star4, star5, PATTERNS, patternArt, shade, rosetteP, postageP, wingP, sc, circ, hexP, SHIELD,
  petalRing, polyP, MODS_TOTAL, TIERS, endorseArt, polyTrim, bezLen, layout, pathPts, ptAt,
  radii, nrm, fitArc, rimRun, ringPt, ringPattern, inspStamp, colourGrid,
};
