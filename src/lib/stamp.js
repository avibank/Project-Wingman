/* =============================================================================
   THE STAMP. ONE RENDERER, EVERYWHERE.
   -----------------------------------------------------------------------------
   Ported from docs/launch/reference/02-licence-stamp-creator.html, which the
   handoff names as the source of truth: "Use inspStamp() from reference/02 as
   THE stamp renderer across the whole app — lesson rows, the sign-off button,
   Up next, Crew walls, profiles and the licence. The older renderer in 01 is
   superseded."

   EVERY PATH, RADIUS AND NUMBER BELOW IS THE DESIGN'S, copied rather than
   rewritten. Six shapes, and each one is its own drawing rather than a scaled
   copy of the others — the handoff says so and the `layout()` entries are why:
   a roundel's rim text runs along a circle, a bag tag's along a rectangle with
   a bite out of it, and a window's along two arcs. Nothing here is generated.

   THREE THINGS WERE ADAPTED, and only three:

     · `MYSTAMP`, the demo's own live stamp, is gone. A stamp is passed in.
     · `inkFilter()` reached into the page's one <svg defs> and appended to it.
       It returns markup now; <StampDefs> renders one filter per seed and every
       stamp that shares a seed shares the filter, which is what §8 asks for
       when a Crew wall draws a hundred of them.
     · `inspStamp` is exported.

   It returns an SVG STRING, like the reference. That is deliberate: the
   drawing is 150 lines of string building and converting it to JSX would mean
   rewriting every path by hand, which is exactly how a port stops matching.
   Stamp.jsx sets it with dangerouslySetInnerHTML — the string is built here
   from a fixed vocabulary and a validated code, never from anything a student
   typed unescaped. `escapeText` is what guarantees that.
   ========================================================================= */

/* THE APP'S, HOISTED ABOVE THE ENGINE because the reference's own renderer
   reaches for both and this file keeps the reference's order — see
   check:stamp, which compares the blocks below character for character. */
export const FONTS = { normal: 'Instrument Sans,ui-sans-serif,system-ui,sans-serif', cursive: 'Pacifico,Brush Script MT,cursive' };
export const inkFilterId = (seed) => `ink2_${seed}`;

/* eslint-disable no-use-before-define -- THE REFERENCE'S OWN ORDER.
   `SHAPES` names `SHIELD`, `hexP` and `sc`, which the reference defines below
   it; every use is inside an arrow body that runs long after the module has
   evaluated, so it is correct at runtime and only reads as a problem to a
   linter walking the file top to bottom. Reordering would make this file stop
   matching the spec it was ported from, which check:stamp exists to enforce. */
export const MOTTO='Never fly alone';
export const DEFAULT_STAMP={shape:'seal',code:'',sym:'tick',ring:'',pattern:'stars',font:'normal',ink:null,seed:1};
...
export const col=(c,l)=>`oklch(${l??c.l??.64} ${c.c} ${c.h})`;
export const SHAPES={
...
export const SYMS=['plane','wrench','prop'];
const inkVal=k=>!k?'var(--accent)':(typeof k==='string'?k:col(k,Math.max(k.l??.64,.58)));
let RID=0;
export const star4=(x,y,r)=>`<path d="M${x} ${y-r}L${x+r*.3} ${y-r*.3}L${x+r} ${y}L${x+r*.3} ${y+r*.3}L${x} ${y+r}L${x-r*.3} ${y+r*.3}L${x-r} ${y}L${x-r*.3} ${y-r*.3}Z" fill="currentColor" stroke="none"/>`;
export const PATTERNS={none:'None',rays:'Rays',checks:'Checks',guilloche:'Guilloche',crochet:'Crochet',knurl:'Knurl'};
...
export function patternArt(p,a,b){let o='';
  ...
  return o}
export const shade=ink=>{if(!ink)return 'color-mix(in oklab,var(--accent) 55%,var(--ground))';const l=ink.l??.64;return `oklch(${(l>.66?l-.2:l+.18).toFixed(2)} ${Math.max(.02,ink.c*.8).toFixed(3)} ${ink.h+(ink.c>.05?12:0)})`};
export function rosetteP(R,n){const pts=[];for(let i=0;i<n;i++){const a=i/n*Math.PI*2-Math.PI/2;pts.push([20+R*Math.cos(a),20+R*Math.sin(a)])}const rb=(R*Math.sin(Math.PI/n)*1.12).toFixed(2);
...
export function postageP(){const a=3,b=37,r=.95,st=3.4;let d=`M${a} ${a}`;const n=Math.floor((b-a)/st),off=(b-a-(n-1)*st)/2;
...
export function wingP(side){const x0=20+side*12.3,f=[[15.2,7.2,1.7],[17.9,6.6,1.6],[20.6,5.6,1.5],[23.2,4.3,1.3],[25.6,2.9,1.1]];
...
export const sc=k=>`translate(20 20) scale(${k}) translate(-20 -20)`;
export const circ=r=>`M${20-r} 20a${r} ${r} 0 1 0 ${2*r} 0a${r} ${r} 0 1 0 ${-2*r} 0Z`;
export const hexP=R=>{const p=[0,60,120,180,240,300].map(d=>{const a=d*Math.PI/180;return `${(20+R*Math.cos(a)).toFixed(2)} ${(20+R*Math.sin(a)).toFixed(2)}`});return 'M'+p.join('L')+'Z'};
export const SHIELD='M20 1.5 36.5 7V19c0 10-7.4 16.6-16.5 19.6C10.9 35.6 3.5 29 3.5 19V7Z';
...
function endorseArt(n,Ly){if(!n||n<1)return '';
  ...
  return o}
export function polyTrim(pts,t){const seg=[];let tot=0;for(let i=1;i<pts.length;i++){const d=Math.hypot(pts[i][0]-pts[i-1][0],pts[i][1]-pts[i-1][1]);seg.push(d);tot+=d}
...
export function bezLen(p){let L=0,px=p[0],py=p[1];for(let k=1;k<=40;k++){const t=k/40,u=1-t,x=u*u*u*p[0]+3*u*u*t*p[2]+3*u*t*t*p[4]+t*t*t*p[6],y=u*u*u*p[1]+3*u*u*t*p[3]+3*u*t*t*p[5]+t*t*t*p[7];L+=Math.hypot(x-px,y-py);px=x;py=y}return L}
...
const _PS={};
/* ADAPTED — A FOURTH TIME, and it is the one the reference could not have
   anticipated. `pathPts` measures a path with the browser's own
   `getTotalLength`/`getPointAtLength`, which needs a real SVG element. The
   demo appended one to `document.querySelector('svg defs').parentNode` — the
   page it lived in had exactly one <svg defs>, and this app does not
   necessarily have any, so that line is a crash waiting for the first screen
   that draws a stamp before a defs block exists.

   It builds its own detached <svg> instead, never touching the document, and
   answers null where there is no DOM at all — which is `npm run check:stamp`,
   running this file in Node. Every caller already falls back to the
   hand-written arcs in `layout()`, because postage and tag use them anyway. */
function pathPts(d){if(_PS[d])return _PS[d];
  ...
  let L=0;try{L=el.getTotalLength()}catch{return null}
  if(!L)return null;
  const N=480,pts=[];
  for(let i=0;i<=N;i++){const q=el.getPointAtLength(L*i/N);pts.push([q.x,q.y])}
  return _PS[d]={pts,N}}
function ptAt(d,t){const P=pathPts(d);if(!P)return null;const {pts,N}=P;const u=((t%1)+1)%1*N,i=Math.floor(u),f=u-i,a=pts[i],b=pts[(i+1)%N];
...
const _RAD={};
/* `pathPts` can answer null (no DOM, or an unmeasurable path); every one of
   these returns null in turn and `inspStamp` falls back to the hand-written
   arc, which is what postage and tag use in every environment. */
function radii(d){const key=d;if(_RAD[key])return _RAD[key];
  const P=pathPts(d);if(!P)return null;
  const {pts,N}=P,M=360,r=new Array(M).fill(0);
...
function rimRun(out,inn,base,len,down,cap){
  const RO=radii(out[0]),RI=radii(inn[0]);
...
export function ringPattern(p,R){const f=v=>v.toFixed(2);let o='';
  ...
  return o}
export function inspStamp(on,size=40,rot=0,st=DEFAULT_STAMP){
  ...
  if(rim&&Ly.rimOut&&Ly.inner){const O=nrm(Ly.rimOut),I=nrm(Ly.rimIn||Ly.inner);
    ...
    /* `rimRun` answers null where a path cannot be measured — no DOM, which is
       check:stamp running this in Node. The hand-written arcs already in
       `layout()` are the fallback, and they are what postage and tag use in
       every environment, so the stamp still draws. */
    if(A&&B){Ly.top=A.d;Ly.tl=A.L;Ly.bot=B.d;Ly.bl=B.L;RZ=[A.f,B.f];
  ...
  return `<svg width="${size}" height="${size}" viewBox="${vb}" aria-hidden="true" ${style}><g transform="rotate(${rot} 20 20)" ${on?`filter="url(#${inkFilterId(st.seed||1)})"`:''} fill="none" stroke="currentColor" ${on?'':'opacity=".45"'} stroke-linejoin="round">${o}</g></svg>`;
}
/* colour palette */


/* --------------------------------------------------------------- the app's
   Everything above this line is the reference's, character for character —
   `npm run check:stamp` holds it against
   docs/launch/reference/02-licence-stamp-creator.html and fails the build on a
   drift. Everything below is the app's own, and each one says why it exists.
   ------------------------------------------------------------------------ */
...
export function validStamp(st) {
  ...
  if (st.ink != null && !PALETTE.some((p) => p.n === st.ink)) return false;
  if (!Number.isInteger(st.seed) || st.seed < 1) return false;
  return true;
}
export const inkByName = (n) => PALETTE.find((p) => p.n === n) || null;
export function inkFilterMarkup(seed) {
  const px = (seed * 37 % 100) / 100;
  return `<filter id="${inkFilterId(seed)}" x="-6%" y="-6%" width="112%" height="112%" color-interpolation-filters="sRGB"><feTurbulence type="fractalNoise" baseFrequency="2.1" numOctaves="2" seed="${seed * 17 + 3}" result="fine"/>
...
/* =============================================================================
   ...
   -----------------------------------------------------------------------------
   inspStamp above is the reference's own signature and is kept that way so
   check:stamp can hold it against the reference. It takes an ink OBJECT, and
   §4 says a stamp stores its ink as a NAME ("Ruby"), because what a name looks
   like is a CSS decision and a stored oklch is a colour nobody can re-tune.
   So this resolves the name, cleans the code and the rim text, and is what
   every component calls. Nothing a student typed reaches the markup without
   passing through cleanCode/cleanRim first.
   ========================================================================= */
export function drawStamp(st, { on = true, size = 40, rot = 0 } = {}) {
  ...
  return inspStamp(on, size, rot, {
    ...
    ink: typeof s.ink === 'string' ? inkByName(s.ink) : s.ink,
  });
}
...
/* =============================================================================
   THE STORED STAMP.
   -----------------------------------------------------------------------------
   0029 keeps it as eight columns on pilot_profiles rather than a blob, so every
   CHECK there mirrors the alphabets above and nothing unrenderable can be
   stored. This is the one place that shape becomes the one the renderer takes.
   ========================================================================= */
