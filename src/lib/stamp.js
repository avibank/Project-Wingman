...
/* The three constants the reference defines just above this block. */
...
/* WHAT A STAMP MAY CONTAIN, and it is checked here as well as on the server.
   §4: "a code of 1-3 characters, A-Z or 0-9, required. There are no symbols",
   rim text at most 10. The drawing builds an SVG STRING, so anything a student
   typed has to be unable to close a tag — the alphabets below are the first
   guard and escapeText is the second. Neither is a substitute for the other:
   the alphabet keeps the design's promise, the escape keeps the markup's. */
export const SHAPE_IDS = ['seal', 'roundel', 'window', 'gauge', 'postage', 'tag'];
export const PATTERN_IDS = ['none', 'rays', 'waves', 'checks', 'swirl', 'guilloche'];
...
/** True when a stamp is something this renderer can draw and a server should accept. */
...
/** The palette entry a stored ink NAME refers to. Names are stored, not values. */
...
/* THE INK FILTER, AS MARKUP RATHER THAN AS A SIDE EFFECT.
   The reference appends a <filter> to the page's one <svg defs> the first time
   a seed is seen. React has no such document to reach into — and §4 asks for
   unique ids and for the defs to be SHARED, because a Crew wall draws a
   hundred stamps and each one would otherwise carry its own copy of the same
   filter. So this returns the id and the markup, <StampDefs> renders one of
   each, and every stamp points at it. The filter itself is unchanged. */
...
export const PATTERNS={none:'None',rays:'Rays',waves:'Waves',checks:'Checks',swirl:'Swirl',guilloche:'Guilloche'};
...
export function inspStamp(on,size=40,rot=0,st=DEFAULT_STAMP){
  ...
  /* `txt` and `barPat` are dead in the reference too, and are kept rather than
     tidied: the FIRST handoff's copy of this file used them — barPat held the
     pattern for a roundel whose rim text ran along a horizontal bar, keyed on
     `Ly.bar` — and the build in the zip, which its README calls the frozen
     spec, dropped that treatment and left the declarations behind. Removing
     them would make this file stop matching the spec it was ported from.
