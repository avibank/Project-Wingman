...
| 2 | high | Licence / Preferences / Appearance | Red "Something's snagged. Try again." banner renders above the content on every profile tab, on a normal load with all network calls returning 200. | **fixed** — owner-verified signed in on all three tabs with an error hook installed: no banner, no console errors, no failed calls. It went with the avatar fix. | 0033 + `<Avatar>` |
| 3 | high | Module | Crew tab does nothing — clicking it leaves you on Lessons. Needs wiring plus the explained empty state. | **fixed** | `9f753a8` (the tab) + `7e1bf74` (`CrewEmpty.jsx`) |
...
| 5 | medium | Licence | Card is wider than its column and overflows the right edge instead of sitting centred under the tab strip. | **fixed** — one 680px column, card 634 inside a 680 box at 1280, measured at all three widths | screen 2 |
| 6 | medium | Licence | "Create your stamp" button overlaps the stamp artwork. | **fixed** — the reference's own `.sblock`/`.ghost`, button centred over the ghost seal on its dashed rule, as the design draws it | screen 2 |
| 7 | medium | Licence | Separate "YOUR CODE" box still present; the code belongs inside the stamp creator and the placeholder should not exist in the codebase. | **fixed** — box deleted, creator opens on the claimed code (verified: reads K4M), placeholder gone from src/scripts/tests/tools/supabase and held by `check:code` | screen 2 |
| 8 | medium | Licence | "YOUR LICENCE / See it as others do" header missing from the top of the card; "See it as others do" stranded at the bottom. | **fixed** — the reference's `.boxh` above the card | screen 2 |
| 9 | medium | Licence | Cover button floats mid-card on the right instead of top-right of the banner. | **fixed** — top-LEFT of the banner, which is where the reference's own `.cvbtn` sits (`left:12px;top:12px`); ADMIN is the top-right badge | screen 2 |
| 10 | medium | Preferences | "Go by callsign" and the "Your pilot / What you'll hear about" notifications block are still present; both were cut. | **fixed** — both deleted outright, `PilotSettings.jsx` with them. Not moved: `identity_display` keeps its default and its readers, and the notice list was a statement, not a setting. | screen 3 |
| 11 | medium | Preferences | "Who greets you" renders name, description, name again — wrong order versus the reference. | **fixed** — choice row, one description line, then the field with that greeter's own placeholder, as 09-preferences.html has it | screen 3 |
| 12 | medium | Module | Library Study cards rows use the quiz doc icon, read "8 cards · 1 saved" and offer "Open" instead of the stacked-card thumbnail, kept-count line, done/total and "Test yourself". | **fixed** — the reference's row: stacked `.cards-thumb` with the count on the front card, "Chapter N cards", "N kept for another look" / "N cards · not started", and **Test yourself**. The one number not drawn is `done/total`: nothing in this app counts flipped cards, and it is stated rather than faked (DECISIONS.md). | screen 4 |
| 13 | low | Module | Search field is narrow and its placeholder is clipped; it should fill the row beside the tabs. | **fixed** — the reference's `label.search`, `flex:1 1 auto`, 780px of a 1054px strip at 1280. Placeholders are the reference's three: "Search lessons" / "Search quizzes, cards and papers" / "Find someone". | screen 4 |
| 14 | low | Module | "6 lessons and 3 quizzes" subtitle missing under the module title; Crew tab has no count badge. | **fixed** — `moduleSubtitle()` under the title, and `<small>` on the Crew tab, both derived. The closing line that said the same sentence at the foot of the list is gone with it. | screen 4 |
...
| 17 | — | Lesson | **Not ported.** The lesson page is still this app's layout, not the reference's. | **fixed** — ported 2026-09-20: the reference's `.lesson` grid, `.col`, `.player`, `.title-row`, `.card.logcard` and `aside.card.next`, in the ordinary 1056px column. ref:diff 12.85% / 23.75% / 25.55%; the geometry matches at 1280 to the pixel and the rest is content the fixture cannot pin (a drawn board against a real video). | screen 5 |
...
The table above is the live-defect tracker that came with the launch pack.
What follows is this repo's own log, kept because several of these were
months old, invisible from the screen, and are the reason particular checks
exist. Numbering is independent of the table.
