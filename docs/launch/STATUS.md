...
| 3 | Crew tab | **done** | | | New screen: `CrewTab.jsx` + `crew.css` + `lib/crew.js`. `PeopleTab` was the question FEED, not this — it keeps its own hidden route. Reachable at `/m/:module/crew` as the third tab. |
