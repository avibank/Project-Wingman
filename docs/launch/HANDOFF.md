...
- **Layout:** desktop and tablet first, phone second. Everything must work at 390px wide with no sideways scroll.
...
- **Cover:**
  ...
  - The set designs are tinted with any of the 36 colours.
  - Image upload needs size and type limits, storage, and a crop that fills 640×128.
- **Photo:** initials by default, or an uploaded photo.
...
- **Stats:** three that everyone fills just by using the app: **Hours flown · Lessons signed off · Days flown**. They come from real data. HOBBS time feeds Hours flown.
...
- Keep it as live, but **move Fly solo here** from the licence page, into the How social box.
- Fly solo must actually hide the student from Crew, the route strip, the radar and presence, and hide others from them.

## 7. Out of scope: don't touch

- **Appearance tab:** stays as it is.
...
.av{background-color:color-mix(in oklab,var(--accent) 55%,oklch(.3 .03 255))!important;letter-spacing:.02em;font-size:30px!important}
...
.stats b{font:600 22px var(--mono)}
  ...
  <div class="col">
    ...
    <section id="t-pref" hidden>
      <div class="box">
        ...
        <p class="ph" id="gName">Wingman</p>
        <p class="pd" id="gDesc">A coworker on the same shift. Notices you're here, never what you scored.</p>
        ...
        <p class="pd" style="margin-top:14px">What Wingman calls you</p>
        <input class="inp" id="callMe" placeholder="Skip it. I'll keep talking until you look up, same as always.">
      </div>
      <div class="box">
...
function layout(shape){
  ...
  if(shape==='roundel'){const r=12.5,g=33*Math.PI/180,pt=a=>`${(20+r*Math.cos(a)).toFixed(2)} ${(20+r*Math.sin(a)).toFixed(2)}`,L=(r*(Math.PI-2*g)).toFixed(2);
    return {fill:circ(16.2),frame:'<rect x="1.2" y="14.8" width="37.6" height="10.4" rx="1.2" stroke-width="1.3" fill="var(--ground)"/><rect x="2.3" y="15.9" width="35.4" height="8.2" rx=".6" stroke-width=".35"/>',inner:'M2.6 16.2H37.4V23.8H2.6Z',band:circ(15.8),
...
function inspStamp(on,size=40,rot=0,st=MYSTAMP){
  ...
  if(p!=='none'){
    const R0=Ly.ring,cx0=ccx,R=(Ly.bar&&rim)?{type:'rect',a:[cx0-1,19.5,cx0+1,20.5],b:[2.8,16.4,37.2,23.6]}:R0.type==='circle'?{type:'circle',ri:1.2,ro:rim?R0.ri-.9:R0.ro}:(p==='rays'?{type:'rect',a:[cx0-1,19,cx0+1,21],b:rim?[R0.a[0]+.8,R0.a[1]+.8,R0.a[2]-.8,R0.a[3]-.8]:R0.b}:(rim?{type:'rect',a:[R0.a[0]+2.2,R0.a[1]+2.2,R0.a[2]-2.2,R0.a[3]-2.2],b:[R0.a[0]+.8,R0.a[1]+.8,R0.a[2]-.8,R0.a[3]-.8]}:R0));
    ...
    const pat=`<mask id="pm${id}" maskUnits="userSpaceOnUse" x="0" y="0" width="40" height="40"><rect width="40" height="40" fill="#fff"/>${halo}</mask><clipPath id="pk${id}"><path d="${Ly.clip||Ly.fill}"/></clipPath><g clip-path="url(#pk${id})"><g mask="url(#pm${id})" opacity="${(p==='checks'?.55:.75)*(rim?.7:1)}">${ringPattern(p,R)}</g></g>`;if(Ly.bar)barPat=pat;else o+=pat}
  o+=sh.o();
  o+=Ly.frame+barPat;
...
const LIVERIES={sky:{n:'Sky blue',h:254,c:.13},amber:{n:'Gauge amber',h:75,c:.13},tarmac:{n:'Tarmac grey',h:255,c:.02},beacon:{n:'Beacon red',h:25,c:.15},runway:{n:'Runway green',h:150,c:.12},skydrol:{n:'Skydrol violet',h:300,c:.13}};
...
const ME={name:'Hassan Alrefaei',callsign:'h.alrefaei',bio:'',phrase:PHRASES[0],cat:'B2 · Avionics',inst:'AU University',admin:true,photo:null,cover:'contour',coverColor:null,coverImg:null,livery:'sky',
  stats:[['13.9','Hours flown'],['7','Lessons signed off'],['12','Days flown']]};
const initials=n=>n.split(/\s+/).map(w=>w[0]).slice(0,2).join('').toUpperCase();
function setBg(c){c=c||PALETTE[1];return `linear-gradient(120deg,${col(c,.5)},oklch(.28 ${(c.c*.75).toFixed(3)} ${c.h}))`}
...
function avatarHTML(){return ME.photo?`<span class="av" style="background-image:url('${ME.photo}');background-size:cover;background-position:center"></span>`:`<span class="av">${initials(ME.name)}</span>`}
...
function paintCard(slam){
  ...
  $('#photo').onclick=pickPhoto;$('#cvBtn').onclick=pickCover;$('#phr').onclick=pickPhrase;
...
function readFile(cb){const i=document.createElement('input');i.type='file';i.accept='image/*';i.onchange=()=>{const f=i.files[0];if(!f)return;const r=new FileReader();r.onload=()=>cb(r.result);r.readAsDataURL(f)};i.click()}
...
function pickPhoto(){
  openSheet(`<h3>Profile picture</h3><div class="pgrid">
    <button class="popt ${!ME.photo?'on':''}" id="useIni"><span class="av sm">${initials(ME.name)}</span><span>Your initials</span></button>
    <button class="popt ${ME.photo?'on':''}" id="upPh"><span class="av sm up">${ME.photo?`<img src="${ME.photo}" alt="">`:'<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 16V5M7 10l5-5 5 5M5 19h14"/></svg>'}</span><span>${ME.photo?'Change photo':'Upload a photo'}</span></button></div>`);
  $('#useIni').onclick=()=>{ME.photo=null;closeV();paintCard();toast('Using your initials')};
  $('#upPh').onclick=()=>readFile(d=>{ME.photo=d;closeV();paintCard();toast('Photo updated')});
}
function pickCover(){
...
let tab='shape';
function openMaker(){draft={shape:'seal',code:'',sym:null,ring:'',rim:true,pattern:'none',ink:PALETTE[1],seed:7,cmode:'code'};confirming=false;tab='shape';paintStudio(true);$('#scrim').classList.add('open')}
...
function paintStudio(anim){
  ...
  $('#shuffle').onclick=()=>{const pick=a=>a[Math.floor(Math.random()*a.length)];draft.shape=pick(Object.keys(SHAPES));draft.pattern=pick(Object.keys(PATTERNS));draft.ink=pick(PALETTE);draft.rim=Math.random()>.35;draft.seed=1+Math.floor(Math.random()*40);confirming=false;paintStudio(true)};
...
$$('.seg button').forEach(b=>b.onclick=()=>{const t=b.dataset.t;if(t==='app'){toast('Appearance stays as it is');return}
  $$('.seg button').forEach(x=>x.setAttribute('aria-selected',x===b));$('#t-lic').hidden=t!=='lic';$('#t-pref').hidden=t!=='pref';$('#pageTitle').textContent=t==='lic'?'Licence':'Preferences'});

/* preferences */
const GREET=[['Wingman',"A coworker on the same shift. Notices you're here, never what you scored."],['The Hermit','Says as little as possible. Still notices you showed up.']];
$$('#greet button').forEach(b=>b.onclick=()=>{$$('#greet button').forEach(x=>x.setAttribute('aria-pressed',x===b));$('#gName').textContent=GREET[+b.dataset.g][0]});
